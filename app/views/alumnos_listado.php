...
<div class="container">
    ...
    <?php if (isset($_GET['status'])): ?>
        ...
        <?php elseif ($_GET['status'] == 'reset_ok'): ?>
            <div class="alert-box alert-success">✨ Tabla Reiniciada correctamente.</div>
        <?php elseif ($_GET['status'] == 'updated'): ?>
            <div class="alert-box alert-info">🔹 Datos del alumno actualizados correctamente.</div>
        <?php elseif ($_GET['status'] == 'deleted'): ?>
            <div class="alert-box" style="background:#feebcb; color:#744210;">🗑️ Registro eliminado satisfactoriamente.</div>
        <?php endif; ?>
    <?php endif; ?>
    ...
    <div class="tools-panel">
        <div>
            ...
            <p style="margin:0; color:#718096; font-size: 0.9em;">Elimine la tabla para subir un nuevo padrón limpio.</p>
        </div>
        <a href="/DatProcesador/alumnos/reiniciarTabla" 
           class="btn btn-danger" 
           onclick="return confirm('⚠️ ¿Eliminar permanentemente a TODOS los alumnos?')">
            🗑️ Reiniciar Tabla Completa
        </a>
    </div>
                ...
                <?php if (!empty($alumnos_duplicados)): ?>
                    <?php foreach ($alumnos_duplicados as $alumno): ?>
                        <tr class="row-error">
                            ...
                            <td><span style="color:#e53e3e; font-weight:bold;"><?php echo htmlspecialchars($alumno['fase'] ?: '---'); ?></span></td>
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr><td colspan="6" style="text-align: center; color: #a0aec0; padding: 20px;">No hay registros duplicados.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
                ...
                <?php if (!empty($alumnos_unicos)): ?>
                    <?php foreach ($alumnos_unicos as $alumno): ?>
                        <tr class="row-success">
                            ...
                            <td><?php echo htmlspecialchars($alumno['nombre_completo']); ?></td>
                            <td style="font-weight:bold; color:#2c5282;"><?php echo htmlspecialchars($alumno['fase'] ?: '---'); ?></td>
                            <td><small><?php echo htmlspecialchars($alumno['descripcion'] ?: 'Sin descripción'); ?></small></td>
                            <td style="text-align: center;">
                                <a href="/DatProcesador/alumnos/editar/<?php echo $alumno['id']; ?>" class="btn-edit">Editar</a>
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr><td colspan="6" style="text-align: center; color: #a0aec0; padding: 20px;">No hay alumnos registrados.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
